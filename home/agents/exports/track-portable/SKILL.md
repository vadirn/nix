...
Tracks live in the repo at `<repo-root>/.tracks/track-<slug>.md`, where `<repo-root>` is the output of `git rev-parse --show-toplevel` (falling back to `pwd` outside a git repo). Active tracks are those whose frontmatter `status` is **not** `done`, `closed`, or `archived`.
