...
Rows whose **Term** is bolded are pinned: text, position, and presence are fixed. Pinned rows describe key anchors a cold reader must resolve before reading the Claim. Append un-pinned rows for working vocabulary; refine an existing un-pinned term by appending a new row with the sharpened wording rather than rewording in place.
