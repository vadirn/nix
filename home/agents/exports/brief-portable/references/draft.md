...
- **artifact** — genuinely read from git (mostly the _shipped_ line and _in progress_).
- **inferred** — your guess from commit cadence. Pre-fill it, but **mark it** so the user knows to check it. Never assert an inferred field as fact.
...
```
// Setup — see SKILL.md "Resolving the project": repo, context, roster.
...
// Gather from git; fold in any context the user gave
commits = Bash(git -C <repo> log --since=<period> --stat)
given   = do("any context the user pasted or pointed at this session (a note, a message,
              a paste) — treated as [elicited], never invented")
...
if user accepts a draft:
    do("set that stakeholder's last_drafted = today in .brief/context.md frontmatter — see Reference: Watermark")
```
...
- no roster → draft a single generic update and offer to create `.brief/context.md` with a `stakeholders:` list (template in SKILL.md).
...
"Drafting for Sarah: counts in features+dates, waits to be told. Stale? edit .brief/context.md".
...
`last_drafted` to today — a surgical edit to that entry in `.brief/context.md` frontmatter. This is
brief's ONLY write. It tracks "last accepted draft" (observable), not "last sent" (which the skill
cannot see). On heavy edits, advance only when done.
...
─────────────────────────────────────
DRAFT — not sent. Send it yourself. Sources: <repo + any context you gave>
```
