...
> 4. Run `distill-text apply <note>.tmp.md`. Report its stderr footer (`— applied: N recovered · M kept · K removed`) and exit code. A refusal (exit 2 stamp/gate mismatch, exit 1 missing key) means nothing was written — surface it, do not retry blindly.
