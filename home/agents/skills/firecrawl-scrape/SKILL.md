...
description: |
  Extract markdown from specific URLs (handles JS/SPAs) via Firecrawl. Routed to from /firecrawl.
allowed-tools:
  - Bash(firecrawl *)
  - Bash(npx firecrawl *)
---
...
- **Scrape to a file and reason over the markdown directly.** Use `grep`, `head`, or read it in full. Use `--query` only when you need a single targeted answer without saving the page (costs 5 extra credits).
