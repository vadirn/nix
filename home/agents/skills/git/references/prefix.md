...
- **PR** — the same unit as its branch. The PR title becomes the commit message on squash-and-merge, so it carries the branch's prefix. Squash flattens the whole branch into that one line on `main`, which makes the PR the atomic unit that actually lands: keep a PR to one concern (a feature and its enabling refactors are one concern; three unrelated chores are three), and keep a revert in its own PR.
