/**
 * Smoke test: magic-byte MIME-mismatch warning path (Fix N3).
 *
 * Run:
 *   bun test imagen-nanobanana.test.ts
 *
...
const TMP_DIR = join(tmpdir(), `nanobanana-test-${process.pid}`);
...
const WORKER = new URL("./imagen-nanobanana.ts", import.meta.url).pathname;

describe("magic-byte MIME mismatch (Fix N3)", () => {
  it("emits WARNING when JPEG bytes are given a .png extension", async () => {
    ...
    expect(stderr).toContain(
      "WARNING: magic-byte MIME (image/jpeg) disagrees with extension MIME (image/png)",
    );
  ...
  it("does not warn when a genuine PNG is passed", async () => {
    ...
    const proc = Bun.spawn(["bun", WORKER, "test prompt", "--source", GENUINE_PNG, "--dry-run"], {
      stdout: "pipe",
      stderr: "pipe",
    });
