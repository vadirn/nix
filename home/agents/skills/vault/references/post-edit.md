...
Skip the `/git commit` suggestion after editing vault content. Cards, notes, references, tracks, and weekly logs reach other devices through Obsidian Sync. Sync alone is enough. The exceptions:

- Edits landed inside `.claude/` or `.scripts/` (confirm with `git status` if unsure).
- The user explicitly asked to commit.

When the exceptions hit, suggest `/git commit` normally.

### Why

Obsidian Sync propagates the vast majority of vault content without git involvement. A `/git commit` prompt after every card or note adds friction that the user has to dismiss every time. It also risks staging files that `.gitignore` would refuse anyway. The git-tracked subtrees are small and obvious from `git status` — when something does land there, the suggestion is informative rather than noise.
