...
| Checkpoint _(legacy — replaced by track)_ | Single-session snapshot recording decisions, frictions, cost, lines written. New work goes to track; existing files remain readable via `vault-query get`. Programmatically treated as superseded: `consult` excludes all checkpoints by default.                                                                            | `41 projects/<project>/`                |
