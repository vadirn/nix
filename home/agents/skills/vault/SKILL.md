...
| Trunk note | A note that is the knowledge trunk behind a promoted skill: the skill holds a compiled copy of the doctrine, the note holds the evolving thinking, and the note's link to the skill is the drift reminder — resync is deliberate, at retraining. Skills never read the vault at run time. Distinct from a plain note: a trunk note has a release cut from it. Registry: the Migration ledger in the "Skill vs note" note. Examples: Design, Implementation, Prototype; Codemod is a demoted skill living as a note. | `00 inbox/` → `30 notes/` |
