...
| Doctrine note | A note holding a skill's doctrine — method, preferences, specimens, output contracts — loaded at invocation by a thin wrapper skill via `vault-query get` + structured `read`. Its section headings are the wrapper's contract: renaming one breaks the skill. Distinct from a plain note: a doctrine note has a machine consumer. Registry: the Migration ledger in the "Skill vs note" note. Examples: Debate, Probe, Grade, Justify, Implementation, Affirm, Pseudocode, Codemod, Design, Glossary, Prototype, Experiment. | `00 inbox/` → `30 notes/` |
