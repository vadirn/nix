...
Answer a fact question at the shallowest depth that closes it. The world settles the fact, not you. Depth is the skill's call, not the caller's. State the question. Get a calibrated answer with citations.
...
| D3    | parallel breadth + adversarial cross-check + grounding of key claims     | a key, contested, or high-stakes fact |
...
Stop when the findings are consistent, complete, and confident enough for the stakes. Stop also at the ceiling. Then synthesize.
...
- **Type boundary** — the "fact" may hinge on a preference or a judgment call. Then stop and hand it back as a **decision**. Keep the choice with the user. Research answers _what is_, never _what to pick_.
