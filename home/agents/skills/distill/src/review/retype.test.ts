...
import { computeSource, type DistillationResult, type Edge, type Unit } from "@/graph/graph.ts";
import { locate } from "@/extract/locate.ts";
import { projectMarkdown, type Projection } from "@/graph/project.ts";
import { type Block, type BlockSpec, parseInteract, renderBlock } from "@/review/interact.ts";
import { applyTyping, buildTypingReview, typingBlockId } from "@/review/retype.ts";
...
test("skip: a declined typing review leaves the projection byte-identical to no review", async () => {
  ...
  const { runTypingReview } = await import("@/app/tty.ts");
  const r = result();
  const before = projectMarkdown(r);
