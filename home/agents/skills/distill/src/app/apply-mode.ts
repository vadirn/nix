...
import { type Item, parseInteract, resolveInteract, stripInteract } from "@/review/interact.ts";
import { TRIAGE_VERBS, safeHandle } from "@/review/triage.ts";
import { askJson, EXTRACT, rethrowIfBug } from "@/kernel/fw.ts";
import {
  fidelityGate,
  renderEntryPrompt,
  verbatimDef,
...
import { parseCanonicalNote, splitSections } from "@/graph/parse-projection.ts";
import { parseFrontmatter } from "@/kernel/frontmatter.ts";
import { detectLang } from "@/kernel/text.ts";
import { stampSha, TRAILING_ANCHOR_RE } from "@/graph/graph.ts";
