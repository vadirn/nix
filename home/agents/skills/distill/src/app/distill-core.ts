...
import {
  ...
  parseSuperseded,
  parseType,
} from "@/kernel/frontmatter.ts";
import { askJson, isTransient, TruncationError } from "@/kernel/fw.ts";
import { extractGraph, gradeBlocks } from "@/prompt/prompts.ts";
import { formatNameLint, nameLintAgainstSource, type NameLintResult } from "@/writing/name-lint.ts";
import { locateGraph, payloadKey } from "@/extract/locate-graph.ts";
...
import { type Residue, edgePayloadResidue } from "@/review/residue.ts";
import { runProse } from "@/app/prose-mode.ts";
import { buildIntermediary } from "@/review/triage.ts";
import { runApply, stampHash } from "@/app/apply-mode.ts";
import { runFidelityBackstop, runProseGate } from "@/review/gates.ts";
import { runTypingReview, runTtySession } from "@/app/tty.ts";
import { USAGE, parseArgs, refusePendingIntermediary, tempMdPath, tmpPathFor } from "@/app/cli.ts";
import { buildPassthroughEnvelope } from "@/app/envelope.ts";
