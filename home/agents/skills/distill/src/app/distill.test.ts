...
import { REL_REGISTRY, ensureEpistemicStatus, slugSegment } from "@/app/distill.ts";
import { parseRelationsBlock } from "@/graph/rel-parse.ts";

// 6 levels up from this dir (src/app → src → distill → skills → agents → home →
// /Users/vadim/nix), then into the vault-query lint dir that holds the canonical JSON.
