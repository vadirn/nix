# English writing passes — reference for the embedded rubric in distill.ts

The condensed passes in `distill.ts` (PASS_EN) are derived from these four passes. This file is the full source; edit it, then re-distill into the constant.
